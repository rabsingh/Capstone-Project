...
def edit_item(request, item_id):
    ...
    return render(request, 'shopping_list_app/edit_item.html',
                  {'form': form, 'item': item})
...
def item_list(request):
    ...
    if filter_form.is_valid():
        ...
        if filter_form.cleaned_data.get('week_beginning'):
            items = items.filter(
                week_beginning=filter_form.cleaned_data['week_beginning']
            )
...
def update_authorised_status(request, item_id):
    ...
    if request.user.is_superuser:
        ...
        messages.success(
            request,
            f'Item "{item.item_name}" is now marked as {status}.'
        )
    else:
        messages.error(
            request,
